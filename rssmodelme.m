...
(*C[t] == Y[t] - G[t], (* aggregate resource constraint w/ adj costs *)*)
...
(*Try interest rate rule with output growht rather than output gap*)
(*
4*Int[t] == (1-taylrho) * ( 4*Log[1/beta]
	+ taylpi * (4*Log[pi[t]]-piBar)
	+ tayly*(Y[t]-Y[t-1]) )
...
parametervalues = {
  ...
  chi0 -> (* 4.73921522897550247856008201926876616780052304558537869572838202224763 *)
    Exp[wrealAIMSS +MUcAIMSS], (* normalize L in the nonstochastic steady state to be 1; note the use of *AIMSS variables here to accomplish this.  The AIMSS suffix denotes the nonstochastic
...
(*Changed this*)
  ...
  consoldelta -> Exp[ytmAIMSS/400] *(1-1/100)  (* consol depreciation, set to make duration of consol =25 yrs *) 
}

(*Imitate those parameter values in paper: Fail*)


loglinearizevars = {A, C, Disp, G, L, MUc, p0, pi, piavg, pricebond, pricebondrn, wreal, Y, zd, zn} ;
(*
loglinearizevars = {A, C, Disp, G, L, MUc, p0, pi, pricebond, pricebondrn, wreal, Y, zd, zn} ;
*)
...
*)
AIMPrecision = 50 ; (* 250 *)
AIMZeroTol = 10^-10 (* 10^-50 *);
(* for psi = 1000, AIMZeroTol = 10^-8 and Precision 80 seems to work *)
...
*)
(*lets try deterministic*)

(*Yes, deterministic steady state is different from stochastic*)
momSubs = {Sigma->1, mom[_,3]->0, mom[A,2]->.01^2, mom[G,2]->.004^2, mom[Int,2]->.004^2, mom[pistar,2]->0^2};



(*AIMSeries[rssmodel,3]*)
...
Print["The steady state term premium is equal to: "];

Print[AIMSeries[rssmodel,3][[tppos]] /.momSubs /.termprem[t] -> termpremss /. AIMSSSubs /. ss /.x_Real:> N[x]]  ;



(*Print[AIMSeries[rssmodel,3][[tppos]] /.momSubs /.x_Real:> N[x]] ;*)

(**I think that we need to revise the steady states from the third order approximation!!!!**)
...
simvars = DeleteDuplicates[Join[ Map[Head, AIMLagVars[rssmodel]], reportvars, {pricebond, ytmrn}]] ; (* variables to simulate *)

Export["impulsetestnames.txt",simvars,"CSV"]

lagvarpos = Range[Length[AIMLagVars[rssmodel]]] ;

reportvarpos = Length[AIMLagVars[rssmodel]] + Range[Length[reportvars]] ;

(*Will probably need to change shock variable*)
(*Simplify to get rid of randomness (make nonstochastic)*)
(*RandomReal creates random variable nperiods by number of shocks long*)

(*This shocks matrix is for stochastic model*)
(*
...
*)
(*Use this for deterministic model*)
...
(*shocks[[50,3]]=.004;*)
shocks[[Range[50,50+x],1]]=.01;

soln = AIMSoln[rssmodel,{},simdegree,AIMZeroTol,AIMPrecision][[Flatten[Map[Position[AIMVarNames[rssmodel], #] &, simvars]]]] ;
...
(*
iterator = Apply[Function,{ {simvars,1}, soln} ];

iterator = Apply[soln, {simvars}];
*)
...
(*Everything below just seems to be cumptuations of means, standard*)
...
stddevs = Map[StandardDeviation,syndata[[reportvarpos]]] * {100, 100, 100, 100, 400, 400, 400, 1, 1}
		(* real variable std devs in percentage points, not logs; inflation std devs in annualized percentage points; interest rate std devs in annualized basis points *)
...
Export["rssmodelAAA.txt", syndata,"Table"]

(*Export["impulsetestnames.csv",simvars,"Table"]*)
(*We should also get simulated steady states*)

simss=syndata[[All,-1]];

(*Export["steadystate.txt",simss,"Table"]*)
